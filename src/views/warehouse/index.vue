<template>
	<div class="app-container" v-loading.fullscreen.lock="Loading">
		...
		<el-table v-loading="listLoading" :data="list" :row-style="{'height':'105px'}" element-loading-text="Loading" style="width: 100%;border-radius:12px;" border fit highlight-current-row :header-row-style="{
																							'height':'54px',
																							'box-shadow': '1px 0px 0px 0px rgba(232, 232, 232, 1), 0px 1px 0px 0px rgba(232, 232, 232, 1)',
																							'font-size': '14px',
																							'font-weight': '500',
			...
			<el-table-column align="center" label="商品信息" min-width="474">
				<template slot-scope="list">
					<div class="flex goods_wrap">
						<div class="goods_img">
							<img :src='list.row.image' alt="" v-if="list.row.image != ''">
						</div>
						<div class="goods_name">{{list.row.name}}</div>
					</div>
				</template>
			</el-table-column>

			<el-table-column label="货号" min-width="140" align="center" :cell-style="{'background':'red'}">
				<template slot-scope="list">
					<span class="numberCell">{{list.row.mun}}</span>
				</template>
			</el-table-column>
			<el-table-column label="库存数量" min-width="140" align="center">
				<template slot-scope="list">
					<span class="numberCell">{{list.row.sum}}</span>
				</template>
			</el-table-column>
			<el-table-column label="库存成本" min-width="140" align="center">
				<template slot-scope="list">
					<span class="moneyCell">￥</span>
					<span class="numberCell">{{list.row.cbmoney | toDecimal}}</span>
				</template>
			</el-table-column>
			<el-table-column label="毒到手总价" min-width="140" align="center">
				<template slot-scope="list">
					<span class="moneyCell">￥</span>
					<span class="numberCell">{{list.row.toxic_price | toDecimal}}</span>
				</template>
			</el-table-column>
			<el-table-column label="毒到手总利润" min-width="140" align="center">
				<template slot-scope="list">
					<span class="moneyCell" :class="list.row.total_toxic_price > 0 || list.row.total_toxic_price == 0?'fc_red':'fc_green'">{{list.row.total_toxic_price > 0 || list.row.total_toxic_price == 0 ? '+':'-'}}￥</span>
					<span class="numberCell" :class="list.row.total_toxic_price > 0 || list.row.total_toxic_price == 0?'fc_red':'fc_green'">{{list.row.total_toxic_price | toDecimal}}</span>
				</template>
			</el-table-column>
			<el-table-column class-name="status-col" label="当前盈亏" min-width="140" align="center">
				<template slot-scope="list">
					<span class="profit_red" :class="list.row.cnmslba > 0 || list.row.cnmslba == 0?'fc_red':'fc_green'">{{list.row.cnmslba > 0 || list.row.cnmslba == 0 ? '+':'-'}}￥</span>
					<span class="profit_red_number" :class="list.row.cnmslba > 0 || list.row.cnmslba == 0?'fc_red':'fc_green'">{{list.row.cnmslba | toDecimal}}</span>
				</template>
			</el-table-column>
		...
		<!-- 日志弹窗 -->
		<el-dialog width="740" custom-class="logs_dialog" :show-close="false" :visible.sync="dialogTableVisible">
			<div class="dialog_title">添加日志</div>
			<img class="close" src="@/assets/warehouse/close.png" @click="dialogTableVisible = false" />
			<el-table :data="logData" style="border-radius:4px;" height="400" border :header-row-style="{
										'height':'54px',
										'font-size': '14px',
										'font-weight': '500',
										'color': '#333333',
				...
				<el-table-column property="type" label="操作类型" width="98" align="center">
					<template slot-scope="logData">
						<span class="logs_table_text">{{logData.row.type}}</span>
					</template>
				</el-table-column>
				<el-table-column property="name" label="名称" width="158" align="center">
					<template slot-scope="logData">
				...
				<el-table-column property="number" label="数量" width="83" align="center">
					<template slot-scope="logData">
						<span class="logs_table_text">{{logData.row.costmun}}</span>
					</template>

				</el-table-column>
				<el-table-column property="price" label="价格" width="103" align="center">
					<template slot-scope="logData">
						<span class="logs_table_text">￥{{logData.row.allprice}}</span>
					</template>

				</el-table-column>
				<el-table-column property="date" label="日期" width="173" align="center">
					<template slot-scope="logData">
		...
		<!-- 搜索弹窗 -->
		<el-dialog width="740" custom-class="search_dialog" :show-close="false" :visible.sync="dialogSearchVisible">
			<div class="flex search_wrap">
				<img src="@/assets/warehouse/search.png">
				<el-input class="search_input" placeholder="请输入货号，例如384665-060" @change="search" v-model="search_key" clearable></el-input>
			</div>
			<!-- 搜索没有数据 -->
			<div class="search_wrap_noData" v-if="searchData == ''">
				<img src="@/assets/warehouse/shop.png">
				<div class="search_tip">搜索的商品未搜索到，点击<span class="search_tip_else" @click="addGoods">自定义添加</span></div>
			</div>
			<!-- 搜索有数据 -->
			<div class="search_wrap_Data" v-else>
				<div class="flex search_data_item" v-for="(item, index) in searchData" :key="index" @click="idStorage(item.id)">
		...
		<!-- 自定义添加商品弹窗 -->
		<el-dialog width="740" custom-class="add_dialog" :show-close="false" :visible.sync="dialogAddVisible">
			<div>
				<img class="close" src="@/assets/warehouse/close.png" @click="hideDialog" />
				<div>
					<el-upload class="goods-uploader" action="/prod-api/index.php/ebapi/index_page/upload" name="filename" :show-file-list="false" :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
						<img v-if="storageData.img" :src="storageData.img" class="avatar">
						<img v-else src="@/assets/warehouse/picture.png" class="avatar" style="width:119px">
					</el-upload>
					<div class="add_title">品名</div>
					<el-input class="addGoods_msg_input" placeholder="请输入商品名称" v-model="storageData.name" clearable></el-input>
					<div class="add_title">货号</div>
					<el-input class="addGoods_msg_input" placeholder="货号，如jk133455" v-model="storageData.huo" clearable></el-input>
					<el-button class="addGoods_btton" @click="storage">添加</el-button>
				</div>
			</div>
		</el-dialog>
		<!-- 添加尺寸弹窗 -->
		<el-dialog width="740" custom-class="size_dialog" :show-close="false" :visible.sync="dialogSizeVisible">
			<div>
				<img class="close" src="@/assets/warehouse/close.png" @click="hideDialog" />
				...
				<div class="size_wrap">
					<div class="size_item" :class="item.isSelect?'size_item_select':''" v-for="(item, index) in sizeList" :key="index" @click="sizeSelect(index)">{{item.value}}</div>
					<el-input class="sizeInput" :class="item.isSelect?'sizeInput_select':''" v-for="(item, indexs) in sizeSelfData" :key="'a'+indexs" @focus="sizeInputFocus(indexs)" v-model="item.value"></el-input>
					<div class="size_item" @click="sizeSelf">自定义</div>
				</div>
				<div class="size_minititle">入货价格（¥）</div>
				<el-input class="sale_peice_input" placeholder="请输入价格" @input="check_price" v-model="addSizeData.price"></el-input>
				<div class="size_minititle">数量</div>
				<div class="add_number_wrap">
					<img src="@/assets/warehouse/minus_number.png" alt="" @click="intMinus">
					<div class="addsize_number">
						<el-input class="sale_num_input" :min="0" @blur="intNumBlur($event)" v-model.number="addSizeData.mun"></el-input>
					</div>
		...
		<!-- 卖出弹窗 -->
		<el-dialog width="740" custom-class="sale_dialog" :show-close="false" :visible.sync="dialogSaleVisible">
			<div>
				<img class="close" src="@/assets/warehouse/close.png" @click="hideDialog" />
				<div class="options_goods_detail">
					<div class="options_img">
				...
				<el-input class="sale_peice_input" placeholder="请输入价格" @input="saleCheck_price" v-model="saleData.price"></el-input>
				<div class="size_minititle">数量</div>
				<div class="add_number_wrap">
					<img src="@/assets/warehouse/minus_number.png" alt="" @click="saleMinus">
					<div class="addsize_number">
						<el-input class="sale_num_input" :min="0" :max='saleMaxnum' @blur="saleNumBlur($event)" v-model.number="saleData.mun"></el-input>
					</div>
					<img src="@/assets/warehouse/add_number.png" alt="" @click="saleAdd">
				</div>
		...
		<!-- 入库卖出弹窗 -->
		<el-dialog width="740" custom-class="option_dialog" :show-close="false" :visible.sync="dialogOptionVisible">
			<div>
				<img class="close" src="@/assets/warehouse/close.png" @click="hideDialog" />
				<div class="options_goods_detail">
					<div class="options_img">
				...
				<!-- 表格 -->
				<el-table v-loading="listLoading" :data="goodSize" :row-style="{'height':'54px'}" height='300' element-loading-text="Loading" style="width: 100%;border-radius:12px;margin-top:21px" border fit highlight-current-row :header-row-style="{
																						'height':'54px',
																						'box-shadow': '1px 0px 0px 0px rgba(232, 232, 232, 1), 0px 1px 0px 0px rgba(232, 232, 232, 1)',
																						'font-size': '14px',
																						'font-weight': '500',
					...
					<el-table-column label="入库单价" min-width="100" align="center" :cell-style="{'background':'red'}">
						<template slot-scope="goodSize">
					...
					<el-table-column class-name="status-col" label="毒到手单码利润" min-width="130" align="center">
						<template slot-scope="goodSize">
							<span class="moneyCell" style="font-size:18px" :class="goodSize.row.toxic_price_profit >= 0?'fc_red':'fc_green'">{{goodSize.row.toxic_price_profit >= 0?'+':'-'}}￥</span>
							<span class="numberCell" style="font-size:18px" :class="goodSize.row.toxic_price_profit >= 0?'fc_red':'fc_green'">{{goodSize.row.toxic_price_profit | toDecimal}}</span>
						</template>
					</el-table-column>
					<el-table-column align="center" prop="created_at" label="操作" min-width="87">
						<template slot-scope="goodSize">
							<el-button type="primary" class="sale_button" @click="saleOption(goodSize.row)">
								卖出</el-button>
						</template>
					</el-table-column>
				</el-table>
	...
	methods: {
		...
		idStorage(id) {
			...
			idStorage({ id: id }).then(res => {
				if (res.code == 200 && res.msg == 'ok' && res.data == 1) {
					that.searchData = [];
					that.search_key = '';
					that.hideDialog();
					that.fetchData();
